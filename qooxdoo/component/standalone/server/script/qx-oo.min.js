...
(function(){var a='"',b="Class '",c="",d="Unsupported type!",e="change",f="qx.data.marshal.Json",g="Array",h="_validate",j="' could not be found.",k="]",l="[",m="set",n="_applyEventPropagation",o="qx.data.model.";qx.Class.define(f,{extend:qx.core.Object,implement:[qx.data.marshal.IMarshaler],construct:function(p){qx.core.Object.call(this);this.__tx=p;}
...
},members:{__tx:null,__ty:function(s){return Object.keys(s).sort().join(a);}
,toClass:function(t,u){this.__tz(t,u,null,0);}
,__tz:function(G,B,v,D){if(!qx.lang.Type.isObject(G)||!!G.$$isString||G instanceof qx.core.Object){if(G instanceof Array||qx.Bootstrap.getClass(G)==g){for(var i=0;i<G.length;i++ ){this.__tz(G[i],B,v+l+i+k,D+1);}
;}
;return;}
;var x=this.__ty(G);if(this.__tD(x,v,D)){return;}
;for(var F in G){this.__tz(G[F],B,F,D+1);}
;if(qx.Class.isDefined(o+x)){return;}
;if(this.__tx&&this.__tx.getModelClass&&this.__tx.getModelClass(x,G,v,D)!=null){return;}
;var H={};var y={__tA:this.__tA};for(var F in G){if(this.__tx&&this.__tx.getPropertyMapping){F=this.__tx.getPropertyMapping(F,x);}
;F=F.replace(/-|\.|\s+/g,c);{}
;H[F]={};H[F].nullable=true;H[F].event=e+qx.lang.String.firstUp(F);if(B){H[F].apply=n;}
;if(this.__tx&&this.__tx.getValidationRule){var A=this.__tx.getValidationRule(x,F);if(A){H[F].validate=h+F;y[h+F]=A;}
;}
;}
;if(this.__tx&&this.__tx.getModelSuperClass){var E=this.__tx.getModelSuperClass(x,v,D)||qx.core.Object;}
else {var E=qx.core.Object;}
;var z=[];if(this.__tx&&this.__tx.getModelMixins){var C=this.__tx.getModelMixins(x,v,D);if(!qx.lang.Type.isArray(C)){if(C!=null){z=[C];}
;}
else {z=C;}
;}
;if(B){z.push(qx.data.marshal.MEventBubbling);}
;var w={extend:E,include:z,properties:H,members:y,destruct:this.__tB};qx.Class.define(o+x,w);}
,__tB:function(){var I=qx.util.PropertyUtil.getAllProperties(this.constructor);for(var J in I){this.__tA(this.get(I[J].name));}
;}
,__tA:function(K){if(!(K instanceof qx.core.Object)){return;}
;if(K.isDisposed()){return;}
;K.dispose();}
,__tC:function(L,P,Q,O){var R;if(this.__tx&&this.__tx.getModelClass){R=this.__tx.getModelClass(L,P,Q,O);}
;if(R!=null){return (new R());}
else {var N=o+L;var M=qx.Class.getByName(N);if(!M){throw new Error(b+N+j);}
;return (new M());}
;}
,__tD:function(S,V,T){var U=this.__tx;return U&&U.ignore&&U.ignore(S,V,T);}
,toModel:function(W){return this.__tE(W,null,0);}
,__tE:function(bj,Y,bf){var bd=qx.lang.Type.isObject(bj);var X=bj instanceof Array||qx.Bootstrap.getClass(bj)==g;if((!bd&&!X)||!!bj.$$isString||bj instanceof qx.core.Object){return bj;}
else if(this.__tD(this.__ty(bj),Y,bf)){return bj;}
else if(X){var bh=qx.data.Array;if(this.__tx&&this.__tx.getArrayClass){var be=this.__tx.getArrayClass(Y,bf);bh=be||bh;}
;var bk=new bh();bk.setAutoDisposeItems(true);for(var i=0;i<bj.length;i++ ){bk.push(this.__tE(bj[i],Y+l+i+k,bf+1));}
;return bk;}
else if(bd){var ba=this.__ty(bj);var bi=this.__tC(ba,bj,Y,bf);for(var bg in bj){var bb=bg;if(this.__tx&&this.__tx.getPropertyMapping){bb=this.__tx.getPropertyMapping(bg,ba);}
;var bl=bb.replace(/-|\.|\s+/g,c);{}
;bb=bl;var bc=m+qx.lang.String.firstUp(bb);if(bi[bc]){bi[bc](this.__tE(bj[bg],bg,bf+1));}
;}
;return bi;}
;throw new Error(d);}
